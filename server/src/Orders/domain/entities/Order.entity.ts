...
export type WhatsAppStatus =  WhatsAppStatusNames.PENDING | WhatsAppStatusNames.SENT | WhatsAppStatusNames.RESPONDED | WhatsAppStatusNames.COMPLETED; 
...
export class Order {
    ...
    calculateTotals(): void {
        ...
        this.shippingCost = this.isFreeShipping ? 0 : this.shippingCost || 0;
        this.total = this.subtotal + (this.shippingCost || 0);
        this.updatedAt = new Date();
    }

    generateWhatsAppMessage(): string {
        const itemsText = this.products.map(item =>
            `- ${item.productName}${item.size ? ` (Talle ${item.size})` : ''}: ${item.quantity} x $${item.unitPrice.toFixed(2)}`
        ).join('\n');

        return `¡Hola ${this.customerName}! 👋\n\n` +
