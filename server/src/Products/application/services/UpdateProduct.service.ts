...
export class UpdateProductService extends Service<[string, CreateProductDto], Product> {
    constructor(
    ...
    async execute(id:string, data: CreateProductDto): Promise<Product> {
        ...
        if (!existingProduct) throw CustomError.notFound("Producto a actualizar no encontrado");

        // 2. Verificar que exista la categoria del producto a crear
        const existingCategory = await this.categoryRepository.getByName(data.categoryId);
        if(!existingCategory) throw new ValidationError({categoryId:['La categoría especificada no existe']})

        //3. Verificar si el nombre ya existe en otro producto
        const productWithSameName = await this.productRepository.getByName(data.name);
        if (productWithSameName && existingProduct.id !== productWithSameName.id) throw new ValidationError({name: ["Ya existe un producto con ese nombre"]});
