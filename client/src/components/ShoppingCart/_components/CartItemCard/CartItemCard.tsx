...
interface CartItemProps {
  ...
  plusItem: (productId: string, size?: string) => void;
}
...
const CartItemCard = ({
  ...
  const isDiscount =
    product.discountPercentage !== undefined && product.discountPercentage;
  return (
    <Card
      key={item.productId}
        ...
        <div className="flex items-start justify-between gap-3">
          ...
          <div className="flex-1 min-w-0 flex flex-col flex-wrap gap-2">
            ...
            <div className="border-[1px] rounded-sm py-0.5 max-w-20 flex items-center justify-between ">
              <Button
                ...
                onClick={() =>
                  removeItem(item.productId, item.size ? item.size : undefined)
                }
              >
          ...
          <div className="flex flex-col items-end gap-1">
            <Button
              ...
              onClick={() =>
                removeProduct(item.productId, item.size ? item.size : undefined)
              }
            >
              <X className="h-3 w-3" />
